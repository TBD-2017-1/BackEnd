...
public class Neo4jClass {
    ...
    public void addMencion(String nodoOrigen, String nodoDestino){
        StatementResult result;
        result = this.session.run("match (a)-[r:tweet]->(b)"
                                +"where a.name="+nodoOrigen+" and b.name="+nodoDestino+""
                                +"return r.mencion as mencion");
        Record record = result.next();
        int menciones = Integer.parseInt(record.get("mencion").toString());
        menciones++;
        this.session.run("match (a)-[r:tweet]->(b)"
                        +"where a.name="+nodoOrigen+" and b.name="+nodoDestino+""
    ...
    private boolean existRelation(String origin, String name){
        StatementResult result;
        result = this.session.run("match (a) where a.name="+name+" return a");
        if(result.hasNext()){
          return true;
        }
        return false;
